...
function setIcon(value = textarea.value) {
  realBrowser.action.setIcon({
    path: 'images/bulb' + (value === '' ? 'Off' : 'On') + (darkIcon ? 'Dark' : 'Light') + '.png'
  });
}
...
const selectionStartLoaded = loadData('selectionStart', 'selectionDate', 0).then(
  value => (textarea.selectionStart = value)
);
const selectionEndLoaded = loadData('selectionEnd', 'selectionDate', 0).then(
...
textarea.addEventListener('keyup', function (e) {
  ...
  let end =
    textarea.value.substring(textarea.selectionStart, textarea.value.length).indexOf('\n') +
    textarea.selectionStart;
  if (end < textarea.selectionStart) {
    end = textarea.value.length;
  }
  check: for (
    let i = Math.max(0, textarea.value.substring(0, textarea.selectionStart).lastIndexOf('\n'));
    i < textarea.value.length - 6;
    i++
  ) {
    for (let j = 0; j < names.length; j++) {
      if (textarea.value.substring(i, i + names[j].length + 1) === '/' + names[j]) {
        for (
          let k = i + names[j].length + 1;
          k < textarea.value.length && textarea.value[k] !== '\n';
          k++
        ) {
          if (textarea.value[k] === '=') {
            let ans;
            switch (j) {
              ...
              case 1: //rand <low> <high> = random number between low and high
                ...
                ans =
                  Math.floor(
                    Math.random() *
                      (textarea.value.substring(m + 1, k - 1) -
                        textarea.value.substring(i + 6, m) +
                        1)
                  ) + parseInt(textarea.value.substring(i + 6, m));
                break;
              ...
              case 3: //d2h <number> = base conversion of number
                ans = parseInt(textarea.value.substring(i + 5, k - 1))
                  .toString(16)
                  .toUpperCase();
                break;
            ...
            textarea.value =
              textarea.value.substring(0, i) +
              textarea.value.substring(i + 1, k + 1) +
              ' ' +
              ans +
              textarea.value.substring(k + 1, textarea.value.length);
